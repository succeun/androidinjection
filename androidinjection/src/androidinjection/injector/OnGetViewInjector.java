...
public class OnGetViewInjector extends MethodInjector {

	@Override
	public Class<? extends Annotation> getTarget() {
		return OnGetView.class;
	}

	@Override
	public void inject(Annotation annotation, MethodInvocation invocation, Object ... params) {
		int[] ids = ((OnGetView)annotation).value();
		for (int id : ids) {
			if (id == 0)
				throw new MobileException("id가 정의 되지 않았거나, 찾을 수가 없습니다.");

			// OnGetView만 Activity에서 정의하므로 강제적으로 Receiver 객체에서 찾는다.
